...
class DiskfreeSensor extends Sensor
  ...
  @meta =
    name: 'Diskfree'
    description: "Test the free diskspace of one or multiple shares."
    category: 'sys'
    level: 1
  ...
  run: (cb = ->) ->
    ...
    , (err, stdout, stderr) =>
      ...
      if p.match /^win/
        ...
        col = stdout.split ','
        value.total = Number col[0]
        value.free = Number col[1]
        ...
        value.share = col[0]
        value.type = col[1]
        value.used = Number col[3]
        value.free = Number col[4]
